...
fn main() {      
    ...
    let (ref_data, transcript_names) = sketch::read_input(t, &ref_file, true);
    let ref_sketch:Arc<RwLock<HashMap<i32, Sketch>>>  = sketch::get_sketches(ref_data, k, s, t);  
    ...
    for (id, chunk) in selected.read().unwrap().iter(){
        output_line.push_str(&(">".to_string() + &id.to_string() + "\n" + &sample[*chunk].get(id).expect("msg") + "\n"));
    }
