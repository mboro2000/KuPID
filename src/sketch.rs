...
pub fn read_input(t:i32, file:&String, encode:bool) -> (Vec<HashMap<i32, String>>, HashMap<i32, String>){
    ...
    (seq_data, read_encoding)
}

//Generate the kmer sketches for a set of nucleotide strings
pub fn get_sketches(read_data:Vec<HashMap<i32, String>>, k:i32, s:f64, t:i32) -> Arc<RwLock<HashMap<i32, Sketch>>> {  
    let max:i64 = (1 << (2*k)) - 1;
    ...
    for i in 0..t{
        ...
        let handle = thread::spawn(move || {
            ...
            for (id, seq) in seq_chunk{       
                let (num_kmers, sketch) = frac_min_hash(&seq, k,s,max, hs);
...
pub fn frac_min_hash(seq:&str, k:i32, s:f64, max:i64, hs:i64) -> (i32, HashMap<i64, Vec<i32>>) {
    let mut label:i64 = 0;
